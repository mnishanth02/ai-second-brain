...
export const createNote = mutation({
  ...
  async handler(ctx, args) {
    ...
    if (args.orgId) {
      ...
      //   if (!hasAccess) {
      //     throw new ConvexError("You do not have permission to create a note in this organization");
      //   }
...
async function assertAccessToNote(ctx: QueryCtx | MutationCtx, note: Doc<"notes">) {
  const userId = (await ctx.auth.getUserIdentity())?.tokenIdentifier;
